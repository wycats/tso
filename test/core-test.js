...
QUnit.test("formatting a basic validation descriptor", assert => {
    ...
    expectFormat([desc("url", { absolute: true }), desc("present")], "(call (url absolute=true) (present))");
    expectFormat([desc("present"), desc("number", { min: 1, max: 4 })], "(call (present) (number min=1 max=4))");
    expectFormat([descOn(["create"], "str")], `(call (str)::on(create))`);
});
function desc(name, options = null) {
    return {
        name,
        validator: () => null,
        options,
...
function descOn(contexts, name, options = null) {
    return {
        name,
        validator: () => null,
        options,
